<x-admin.layout>
    ...
    <x-slot name="heading">Bank</x-slot>
                        ...
                        <div class="card-body">
                            <div class="mb-3 row">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <input class="form-control" id="initial" name="initial" type="text" placeholder="Enter Bank Initial">
                                ...
                                <tbody>
                                    @foreach ($bankList as $list)
                                        <tr>
                                            ...
                                            <td>{{ $list->initial }}</td>
                                            <td>
                                                <button class="edit-element btn text-secondary px-2 py-1" title="Edit Bank" data-id="{{ $list->id }}"><i data-feather="edit"></i></button>
                                                <button class="btn text-danger rem-element px-2 py-1" title="Delete Bank" data-id="{{ $list->id }}"><i data-feather="trash-2"></i> </button>
                                            </td>
                                        </tr>
                                    @endforeach
                            </table>
...
<script>
    $("#buttons-datatables").on("click", ".edit-element", function(e) {
        ...
        var url = "{{ route('bank.edit', ":model_id") }}";
        ...
        $.ajax({
            ...
            success: function(data, textStatus, jqXHR) {
                ...
                if (!data.error)
                {
                    ...
                    $("#editForm input[name='initial']").val(data.bank.initial);
                }
                else
                {
                    alert(data.error);
...
<script>
    $(document).ready(function() {
        $("#editForm").submit(function(e) {
            ...
            var url = "{{ route('bank.update', ":model_id") }}";
            //
            $.ajax({
                url: url.replace(':model_id', model_id),
                type: 'POST',
